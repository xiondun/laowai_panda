            ...
            if data['images']:
                for j, datum in enumerate(data['images']):
                    if not os.path.exists('/var/www/api/laowai_panda' + datum['image']):
                        if t_timezone == 'CN':
                            data['images'][j]['image'] = 'https://45.13.199.57' + datum['image']  # 德國
                        elif t_timezone == 'EG':
                            data['images'][j]['image'] = 'https://121.40.208.210' + datum['image']  # 杭州
            ...
            for i, item in enumerate(data):
                if data[i]['images']:
                    for j, datum in enumerate(data[i]['images']):
                        if not os.path.exists('/var/www/api/laowai_panda' + datum['image']):
                            if t_timezone == 'CN':
                                data[i]['images'][j]['image'] = 'https://45.13.199.57' + datum['image']  # 德國
                            elif t_timezone == 'EG':
                                data[i]['images'][j]['image'] = 'https://121.40.208.210' + datum['image']  # 杭州
