...
class YandexMetrikaClient:
    ...
    def api_stat_traffic_summary(self, counter, date1, date2, group=None):
        if group is None:
            group = 'day'
        return self.api_request('/stat/v1/data?ids=%s&date1=%s&date2=%s&group=%s&metrics=ym:s:users,ym:s:visits,ym:s:pageviews' % (
            counter,
    ...
    def api_stat_traffic_summary_bytime(self, counter, date1, date2, group=None):
        if group is None:
            group = 'day'
        return self.api_request('/stat/v1/data/bytime?ids=%s&date1=%s&date2=%s&group=%s&metrics=ym:s:users,ym:s:visits,ym:s:pageviews' % (
            counter,
            date1.strftime('%Y%m%d'),
            date2.strftime('%Y%m%d'),
            group
...
class YandexMetrikaBase(DashboardModule):
    ...
    def api_stat_traffic_summary_bytime(self, group=None):
        if self.counter_attached():
            date1 = datetime.datetime.now() - datetime.timedelta(days=self.period)
            date2 = datetime.datetime.now()

            client = YandexMetrikaClient(self.access_token)
            result, exception = client.api_stat_traffic_summary_bytime(self.counter, date1, date2, group)
    ...
    def get_key_index(self, key=None):
        if key =='visitors':
            return 0
        elif key =='visits':
            return 1
...
class YandexMetrikaVisitorsTotals(YandexMetrikaBase):
    ...
    def init_with_context(self, context):
        ...
        if result is not None:
            try:
                self.children.append({'title': _('visitors'), 'value': result['totals'][0]})
                self.children.append({'title': _('visits'), 'value': result['totals'][1]})
                self.children.append({'title': _('views'), 'value': result['totals'][2]})
            except KeyError:
...
class YandexMetrikaVisitorsChart(YandexMetrikaBase):
    ...
    def init_with_context(self, context):
        result = self.api_stat_traffic_summary_bytime(self.group)

        if result is not None:
            try:
                for i,date in enumerate(result['time_intervals']):
                    date = datetime.datetime.strptime(date[0], '%Y-%m-%d')
                    key = self.get_key_index(self.show )
                    self.children.append((date, result['data'][0]['metrics'][key][i]))
            except KeyError:
                self.error = _('Bad server response')
        ...
        if result is not None:
            try:
                for i,date in enumerate(result['time_intervals']):
                    ...
                    data = {
                            "visitors":result['data'][0]['metrics'][0][i],
                            "visits":result['data'][0]['metrics'][1][i],
                            "page_views":result['data'][0]['metrics'][2][i]
                            }
